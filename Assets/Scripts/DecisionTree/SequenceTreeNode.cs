...
public class SequenceTreeNode<T> : IDecisionNode<T>
{

    private readonly IDecisionNode<T>[] m_nodes;
    private IDecisionNode<T> m_CurrentRunningNode;

    public SequenceTreeNode(IDecisionNode<T>[] allNodes)
    {
        m_nodes = allNodes;   
    }
    ...
    public DecisionTreeStatus Tick(long TickTime, T context)
    {
        ...
        DecisionTreeStatus accumulator = DecisionTreeStatus.Success;
